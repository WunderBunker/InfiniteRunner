...
public class LanesManager : MonoBehaviour
{
    ...
    public byte GetLaneFromXPos(float pXPos)
    {
        float vDistanceMin = math.abs(_lanes[0].CenterX - pXPos);
        byte vLane = 0;
        for (byte lCptLane = 0; lCptLane < _lanes.Length; lCptLane++)
        {
            float lDist = math.abs(_lanes[lCptLane].CenterX - pXPos);
            if (lDist < vDistanceMin)
            {
                vDistanceMin = lDist;
                vLane = lCptLane;
            }
